...
interface IStakeManager {
  ...
  event Withdrawn(address indexed account, address withdrawnAddress, uint256 amount);

  // emitted when stake or unstake delay are modified
  event StakeLocked(address indexed account, uint256 totalStaked, uint256 unstakeRelaySec);

  // emitted once a stake is schedule for withdrawal
  event StakeUnlocked(address indexed account, uint256 withdrawTime);

  event StakeWithdrawn(address indexed account, address withdrawnAddress, uint256 amount);
