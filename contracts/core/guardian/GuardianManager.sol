...
contract GuardianManager is Initializable, UUPSUpgradeable, ISignatureValidatorConstants {
  ...
  modifier onlyOwner() {
    require(msg.sender == owner || msg.sender == address(account), "only owner");
    _;
  }
   ...
   * @param _account the account addess
   * @param _accountFactory the account factory address
   */
  function initialize(address _executor, Account _account, AccountFactory _accountFactory) public initializer {
    executor = _executor;
    account = _account;
  ...
  function setupGuardians(address[] memory _guardians, uint256 _threshold) public onlyOwner {
    ...
    require(_threshold > 0, "GuardianManager::setupGuardians: _threshold must be bigger than 0.");
    require(
    ...
    for (uint256 i = 0; i < _guardians.length; i++) {
      ...
      require(!guardians[guardian], "GuardianManager::setupGuardians: guardian already existed.");
  ...
  function setThreshold(uint256 _threshold) public onlyExecutor {
    require(threshold > 0, "GuardianManager::setThreshold: threshold haven't been setup yet.");
    require(
  ...
  function addGuardian(address _guardian) public onlyExecutor {
    require(threshold > 0, "GuardianManager::addGuardian: threshold haven't been setup yet.");
    require(
      _guardian != address(0) && _guardian != address(this),
      "GuardianManager::addGuardian: invalid guardian address."
    );
    require(!guardians[_guardian], "GuardianManager::addGuardian: guardian already existed.");
  ...
  function removeGuardian(address _guardian) public onlyExecutor {
    require(threshold > 0, "GuardianManager::removeGuardian: threshold haven't been setup yet.");
    require(
      _guardian != address(0) && _guardian != address(this),
      "GuardianManager::removeGuardian: invalid guardian address."
    );
    require(guardians[_guardian], "GuardianManager::removeGuardian: guardian not existed.");
    require(
  ...
  function changeOwner(
    ...
    require(checkMultisig(dataHash, data, signatures), "GuardianManager::changeOwner: invalid multi sig");
    accountFactory.changeOwner(account, data);
