...
import { AccountFactory, Account__factory, ERC1967Proxy__factory, EntryPoint } from "../typechain";
import { UserOperation } from "./UserOperation";
import { AddressZero, callDataCost } from ".";
...
export function signUserOp(
  ...
  const sig = ecsign(keccak256_buffer(msg), Buffer.from(arrayify(signer.privateKey)));
  const signedMessage = toRpcSig(sig.v, sig.r, sig.s);
  return { ...op, signature: signedMessage };
}
