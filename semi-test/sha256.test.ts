...
describe('DeepEvaluation', function () {
  it('Evaluate time to generating proof (sha256 hash function)', async function () {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      const { proof, publicSignals } = await groth16.fullProve(
        ...
        './circom/sha256-test_js/sha256-test.wasm',
        './circom/sha256-test1.zkey'
      );
      const endTime = performance.now();
      times.push(endTime - startTime);
      const startTime1 = performance.now();
      const vKey = JSON.parse(readFileSync('./circom/sha256-test_verification_key.json', 'utf-8'));
